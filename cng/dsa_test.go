...
func TestGenerateDSAParameters(t *testing.T) {
	testGenerateDSAParameters(t, 1024, 160, cng.NewSHA1())
	testGenerateDSAParameters(t, 2048, 256, cng.NewSHA256())
	testGenerateDSAParameters(t, 3072, 256, cng.NewSHA256())
}

func testGenerateDSAParameters(t *testing.T, L, N int, h hash.Hash) {
	...
	testDSASignAndVerify(t, L, priv, h)
}

func testDSASignAndVerify(t *testing.T, i int, priv *cng.PrivateKeyDSA, h hash.Hash) {
	hashed := []byte("testing")
	h.Write(hashed)
	hashed = h.Sum(nil)
...
func TestDSASignAndVerify(t *testing.T) {
	...
	testDSASignAndVerify(t, 0, priv, cng.NewSHA1())
}
