...
// TestHKDFSHA3 verifies HKDF with SHA-3 on Windows versions where CNG exposes
// SHA-3 (Windows 11 24H2 / Server 2025+); it skips on older Windows. The inputs
// are RFC 5869 Test Case 1. RFC 5869 publishes no SHA-3 vectors, so the expected
// PRK and OKM were generated with Go's crypto/hkdf over crypto/sha3.
func TestHKDFSHA3(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cng.NewSHA3_256,
			[]byte{
				0x7d, 0x41, 0x94, 0x83, 0x6f, 0x7a, 0x11, 0x3a,
				0x44, 0x67, 0x7a, 0xbc, 0x82, 0x56, 0x40, 0xad,
				0xe0, 0x7a, 0xf1, 0xc1, 0xd6, 0x9a, 0x9a, 0x4b,
				0x10, 0x9b, 0x28, 0x0a, 0x8f, 0xe5, 0x4e, 0xf0,
			},
			[]byte{
		...
		},
		{
			...
			cng.NewSHA3_384,
			[]byte{
				0x78, 0x55, 0xbc, 0x93, 0x00, 0xa4, 0xdb, 0x53,
				0x2c, 0x9c, 0xab, 0x25, 0x93, 0x79, 0x6e, 0x1a,
				0x4b, 0xbb, 0x77, 0xa2, 0x4d, 0x41, 0x7e, 0x66,
				0x82, 0x2b, 0xea, 0xa3, 0x6f, 0xab, 0xd4, 0x12,
				0x51, 0x5d, 0xcf, 0x38, 0x88, 0x10, 0xad, 0xf2,
				0x7f, 0xa2, 0x3d, 0x3d, 0x7d, 0xef, 0x84, 0xca,
			},
			[]byte{
		...
		},
		{
			...
			cng.NewSHA3_512,
			[]byte{
				0xe1, 0xc5, 0x43, 0x09, 0x4f, 0x64, 0xf3, 0xd6,
				0xc6, 0x65, 0x8a, 0x94, 0xa9, 0x4e, 0x38, 0x18,
				0xba, 0x13, 0xd0, 0xb3, 0xe7, 0x70, 0x74, 0xb8,
				0x0f, 0x88, 0xf3, 0x2e, 0x6b, 0x84, 0x33, 0xb7,
				0x03, 0x53, 0x6c, 0xb5, 0x00, 0x75, 0x39, 0x67,
				0xfa, 0xe2, 0xea, 0x97, 0x7e, 0x11, 0xe4, 0xdd,
				0x4f, 0x45, 0x38, 0x98, 0x07, 0xcd, 0xf2, 0x55,
				0xb3, 0x95, 0xe4, 0x68, 0x07, 0xc8, 0x7d, 0x5d,
			},
			[]byte{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			prk, err := cng.ExtractHKDF(tt.hash, master, salt)
			if err != nil {
				t.Fatalf("error extracting HKDF: %v", err)
			}
			if !bytes.Equal(prk, tt.prk) {
				t.Errorf("incorrect PRK: have %x, want %x", prk, tt.prk)
			}
			out, err := cng.ExpandHKDF(tt.hash, prk, info, len(tt.out))
			if err != nil {
				t.Fatalf("error expanding HKDF: %v", err)
			}
			if !bytes.Equal(out, tt.out) {
				t.Errorf("incorrect output: have %x, want %x", out, tt.out)
			}
		})
	}
}
